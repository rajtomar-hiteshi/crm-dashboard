...
export default function MultiLineChart({ data, lines, xKey = 'month', height = 300, onPointClick }) {
  ...
  const tickFormatter = isMonth ? fmtMonth : isDate ? fmtChartDate : undefined
  const tickInterval = isDate ? getTickInterval(data.length) : 0
    ...
    return (
      <div className="rounded-lg p-3 shadow-xl border" style={{ background: chartColors.tooltipBg, borderColor: chartColors.tooltipBorder }}>
        <p className="text-sm font-medium mb-2" style={{ color: chartColors.tooltipText }}>
          {isMonth ? fmtMonth(label) : isDate ? fmtChartDate(label) : label}
        </p>
        {payload.map((entry, i) => (
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: isDate ? 40 : 5 }} onClick={handleClick}>
